...
|| empty($s_password) || empty($s_confirm_password)|| empty($s_security_question)|| empty($s_security_answer)|| empty($s_dob)) {
  ...
  exit();
    ...
    if ($result_check > 0) {
        ...
        $query = "INSERT INTO student_table (srn, name, department_id,semester,section,email,password,security_question,security_answer,dob)
        ...
        if (!$result) {
        $m=$s_srn." ".$s_name." ".$s_department." ".$s_semester." ".$s_section." ".$s_email.$hash_password." ".$s_security_question." ".$s_security_answer." ".$s_dob;

          header("Location: registerform.php?message=register_fail".$m);
        }else{
