  ...
  <div class="">
    ...
    <section class="ab-info-main py-md-5 py-4">
        <div class="container py-md-5 py-4">
            <h3 class="tittle text-center mb-lg-5 mb-3"><span class="sub-tittle">Reset</span>Password here</h3>
              ...
              <div class="col-lg-5">
                <form method="post">
                  ...
                  <div class="form-group">
                    <select class="form-control" name="teacher_select_question">
                  ...
                  <div class="form-group">
                    <input class="form-control" type="text" name="teacher_enter_answer" placeholder="enter the answer" required>
                  </div>
                  <div class="form-group">
                    <input class="form-control" type="text" name="new_password" placeholder="Enter New Password">
                  </div>
                  <div class="form-group">
                    <input class="form-control" type="text" name="new_confirm_password" placeholder="Confirm New Password">
                  </div>


                  <button type="submit" name="get_password_button" class="btn btn-primary submit mb-4">Reset</button>
...
<?php
    if (isset($_POST['get_password_button'])) {
      $question=$_POST['teacher_select_question'];
      $answer=$_POST['teacher_enter_answer'];
      $new_p=$_POST['new_password'];
      $new_c_p=$_POST['new_confirm_password'];
      $id=$_SESSION['teacher_id_session'];

      if(empty($question) || empty($answer) || empty($new_p) || empty($new_c_p)){
        header("Location: teacher_forgot_password.php?message=Reset_FAILED");
        exit();
      }elseif ($new_p!=$new_c_p) {
        header("Location: teacher_forgot_password.php?message=Reset_FAILED");
        exit();
      }
      $hash_password=password_hash($new_p,PASSWORD_DEFAULT);
        $sql="UPDATE professor SET teacher_password='$hash_password' WHERE security_question='$question' and security_answer='$answer' and teacher_id='$id'";
        $update_sql=mysqli_query($conn, $sql);

        if (!$update_sql) {
            echo '<script language="javascript">';
            echo 'alert("Reset Failed")';
            echo '</script>';
        } else {
            echo '<script language="javascript">';
            echo 'alert("Your Password is Reset")';
            echo '</script>';
        }
    }
?>
