  ...
  <div class="">
    ...
    <section class="ab-info-main py-md-5 py-4">
        <div class="container py-md-5 py-4">
            <h3 class="tittle text-center mb-lg-5 mb-3"><span class="sub-tittle">Add</span>Department here</h3>
              ...
              <div class="col-lg-5">
                <form method="post">
                  <div class="form-group">
                    ...
                    <input class="form-control" type="text" name="department_name" placeholder="Enter Department Name" required>
                  </div>


                  <button type="submit" name="add_department_button" class="btn btn-primary submit mb-4">ADD</button>
...
<?php
if (isset($_POST['department_name'],$_POST['add_department_button'])) {
    $d_name=strtolower($_POST["department_name"]);

    // validation
    if (empty($d_name)) {
        header('Location: add_department.php?message=Empty_fields');
        exit();
    }
    if (!preg_match("/[a-zA-Z]/i", $d_name)) {
        header('Location: add_department.php?message=Invalid_department_name');
        exit();
    }

    $sql = "INSERT INTO `department` (`department_name`) VALUES ('$d_name')";
    $result=mysqli_query($conn, $sql);

    if (!$result) {
        echo '<script language="javascript">';
        echo 'alert("Invalid department name")';
        echo '</script>';
        unset($_POST);
    } else {
        echo '<script language="javascript">';
        echo 'alert("Successfully inserted the department")';
        echo '</script>';
        unset($_POST);
    }
}
