...
<body>
  <div class="">
    ...
    <section class="ab-info-main py-md-5 py-4">
        <div class="row justify-content-center container py-md-5 py-4">
            <div class="col-lg-5">
            <h3 class="tittle text-center mb-lg-5 mb-3"><span class="sub-tittle">Password Reset</span>Student</h3>
            ...
            <div class="login p-md-5 p-4 mx-auto bg-white mw-100">
                ...
                <form method="post">
                  <div class="form-group">
                    ...
                    <input class="form-control" type="text" name="srn_reset" placeholder="Enter SRN">
                  </div>


                  <button type="submit" name="reset_student_password_button" class="btn btn-primary submit mb-4">Reset Now</button>
  ...
  <h3 class="tittle text-center mb-lg-5 mb-3"><span class="sub-tittle">Password Reset</span>Teacher</h3>

  <div class="login p-md-5 p-4 mx-auto bg-white mw-100">
      <form method="post">
        <div class="form-group">
          <label>Enter Id</label>
          <input class="form-control" type="text" name="teacher_id_reset" placeholder="Enter Id">
        </div>


        <button type="submit" name="reset_teacher_password_button" class="btn btn-primary submit mb-4">Reset Now</button>
...
<?php
if (isset($_POST['reset_student_password_button']) || isset($_POST['srn_reset'])) {
        $s_srn=strtolower(mysqli_real_escape_string($conn, $_POST['srn_reset']));
        $s_password="defaultpassword";
        $hash_password= password_hash($s_password, PASSWORD_DEFAULT);

        $sql="UPDATE student_table SET password='$hash_password' WHERE srn='$s_srn'";
        $update_sql=mysqli_query($conn, $sql);
        $count=mysqli_query($conn, "SELECT * FROM student_table WHERE srn='$student_id'");
...
if (isset($_POST['reset_teacher_password_button']) || isset($_POST['teacher_id_reset'])) {
    $t_id=strtolower(mysqli_real_escape_string($conn, $_POST['teacher_id_reset']));
    $t_password="defaultpassword";
    $hash_password= password_hash($t_password, PASSWORD_DEFAULT);

    $sql="UPDATE professor SET teacher_password='$hash_password' WHERE teacher_id='$t_id'";
    $update_sql=mysqli_query($conn, $sql);
    $count=mysqli_query($conn, "SELECT * FROM professor WHERE teacher_id='$teacher_id'");
    if (mysqli_num_rows($count)==0) {
        echo '<script language="javascript">';
